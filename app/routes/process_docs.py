...
def pending_document_ids():
    ...
    if not docs:       
        ...
        return jsonify({"doc_id":None,"message_id":None,"upload_type":None,"file_name":None,"s3_path":None}), 200
    doc_ids=[]
...
def add_document():
    try:
        ...
        system_generated=request.form.get("system_generated")
        ...
        if system_generated.lower() == "true":
            unique_name = filename.replace(" ", "")
            logger.info(f"System generated file, using original filename | unique_name={unique_name}")
        else:
            
            unique_name=f"mail_{datetime.now().strftime('%y%m%d%H%M%S%f')[:-3]}.pdf"
        ...
        # Upload to S3
        s3_path = mail_upload_to_s3(io.BytesIO(pdf_file_bytes), filename,timestamp,system_generated)
 
        # Save to DB
        doc = Document(
            doc_type_code="N/A",
            doc_filename=unique_name,
...
def upload_attachment():
    try:
        ...
        logger.info(f"Received file upload | filename={filename} | timestamp={time_stamp}")
        ...
        # Extract only date
        date = dt.date()
        logger.info(f"Parsed timestamp | original={time_stamp} | date={date} | unique_name={unique_name}")

 
        S3_BUCKET = current_app.config["AWS_BUCKET"]
        logger.info(f"Uploading file to S3 | bucket={S3_BUCKET} | key={unique_name}")
        s3 = boto3.client(
            "s3",
        ...
        logger.info(f"Using mail folder from config: {mail_folder}")
        ...
        logger.info(f"File uploaded to S3 successfully | bucket={S3_BUCKET} | key={s3_key}")
        return jsonify({
            "status": "success",
            "bucket": S3_BUCKET,
            "path": s3_key
