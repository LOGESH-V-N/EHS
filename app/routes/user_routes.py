...
def add_user():
    ...
    role_ids    = data.get("role_id")            # list of role ids
    ...
    user = User(
        ...
        created_by=g.user_id,   # later you can take this from JWT/current user
    ...
    db.session.execute(
        ...
        """),
        {"user_id": user.uid, "role_id": role_ids}
    )
...
def get_user():
    ...
    role_links = UserRole.query.filter_by(user_id=user.uid).first()
    role_ids = role_links.role_id # uses tbl_user_roles.role_id
...
def update_user():
    ...
    user.updated_by = g.user_id   # later: current user from JWT
    ...
    db.session.add(UserRole(user_id=uid, role_id=role_ids))
