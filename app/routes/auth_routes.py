...
def login():
    ...
    # ------------------------------------------------------
    user_roles = UserRole.query.filter_by(user_id=user.uid).all()
    if not user_roles:
    ...
    # ------------------------------------------------------
    privileges = set()
    resource_access = 1  # default full access if any role is type 1

    for role in roles:
        if role.resource_access == 1:
            resource_access = 1  # at least one role has full access
        else:
            # Get modules assigned to this role
            role_privs = RolePrivilegeMap.query.filter_by(role_id=role.id).all()
