...
def login():
    ...
    if user.status == 1:
        return jsonify({"status": 0, "message": "User is inactive"}),  200
  ...
  # default full access if any role is type 1
    ...
    # ------------------------------------------------------
    # 5️⃣ Build REFRESH TOKEN (Long expiry)
    # ------------------------------------------------------
    ...
    refresh_payload = {
        ...
        "exp": datetime.datetime.utcnow() + timedelta(hours=36)
    }
...
@auth_bp.route("/micro_service/authorization", methods=["POST"])
def micro_service_authorization():
    try:
        return jsonify({"status": 1, "message": "Authorization successful"}), 200
    except Exception as e:
        return jsonify({"status": 0, "message": str(e)}), 500
 

@auth_bp.route("/ingestion/login", methods=["POST"])
def ingestion_login():
    try:
        payload = {
            "rules": {
...
def refresh():
    ...
    try:
        ...
        user_roles = UserRole.query.filter_by(user_id=user.uid).first()
        if not user_roles:
            return jsonify({ "message": "No roles assigned to user"}), 200
        ...
        roles = RoleMaster.query.filter_by(id=user_roles.role_id).first()
        if not roles:
            return jsonify({ "message": "Roles not found"}), 200
    ...
    # default full access if any role is type 1
        ...
        new_access_token = jwt.encode(access_payload, JWT_SECRET, algorithm=JWT_ALGO)
