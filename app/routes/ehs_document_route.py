...
from app.utils.document_download import local_download_from_s3
...
def download_multiple_documents():
    try:
        ...
        if not doc_ids or not isinstance(doc_ids, list):
            return jsonify({
                "status": 0,
                "message": "doc_ids must be a list"
            }), 200
        ...
        for doc in documents:
            if doc.doc_file_path:
                try:
                    file_data = local_download_from_s3(doc.doc_file_path,doc.doc_filename)
