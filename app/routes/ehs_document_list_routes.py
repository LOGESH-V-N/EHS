...
def get_document_with_base64_and_json():
    try:
        ...
        # BASE64 FILE
        # -------------------------------------
        file_data = download_from_s3_as_base64(doc.doc_file_path)
        ...
        # =====================================================
        # ✅ INTEGRATIONS — independent of doc_status
        # =====================================================
        integrations = []
        try:
            active_m_ids = [
                row.m_id
                for row in db.session.query(AppModules.m_id)
                .filter(AppModules.active_status == 1)
                .all()
            ]
            ...
            if active_m_ids:
                integrations = [
                    row.module_name
                    for row in db.session.query(IntegrationModules.module_name)
                    .filter(IntegrationModules.m_id.in_(active_m_ids))
                    .all()
                ]
        except Exception as ex:
            print("Integrations error:", str(ex))
            integrations = []
        ...
        result = {
            ...
            # ✅ Always returned regardless of doc_status
...
# Example: 21/11/2025 08:02

def list_date_format(dt: datetime | None) -> str | None:
    """
...
def get_document_list():
    try:
        ...
        if search_term:
            query = query.join(
                DocumentListSchema,
                DocumentListSchema.id == Document.patient_id,
                isouter=True
            ).filter(
                or_(
                    Document.doc_filename.ilike(f"%{search_term}%"),
                    DocumentListSchema.patient_name.ilike(f"%{search_term}%"),
                    cast(DocumentListSchema.dob, String).ilike(f"%{search_term}%"),
                    DocumentListSchema.sex.ilike(f"%{search_term}%"),
                    cast(DocumentListSchema.nhs_no, String).ilike(f"%{search_term}%")  
                )
            )
        ...
        for doc in documents:
            ...
            if doc.doc_status in [3, 4, 5, 6] and doc.patient_id:
                ehs_patient = (
                    db.session.query(
                        DocumentListSchema.patient_name,
                        DocumentListSchema.dob,
                        DocumentListSchema.sex
                    )
                    .filter(DocumentListSchema.id == doc.patient_id)
                    .first()
                )
                ehs_nhs_no = (db.session.query(DocumentListSchema.nhs_no).filter(DocumentListSchema.id == doc.patient_id).first())

                nhs_number = ehs_nhs_no.nhs_no if ehs_nhs_no else ""

                if ehs_patient:
                    name = ehs_patient.patient_name or ""

                    dob = ""
                    if ehs_patient.dob:
                        if hasattr(ehs_patient.dob, "strftime"):
                            dob = format_datetime(ehs_patient.dob)
                        else:
                            dob = str(ehs_patient.dob)

                    sex = ehs_patient.sex or ""
                    patient_info = " ".join(filter(None, [name, dob, sex]))
            ...
            if doc.doc_status in [3, 4, 5, 6]:
                processed_log = (
                    ...
                    .filter(
                        ...
                        Log.doc_status.in_([3, 4, 5, 6])  # ✅ Check any processed status log
                    )
                    .order_by(Log.datatime.desc())
                    .first()
                )

                if processed_log and processed_log.datatime:
                    processed_date = list_date_format(processed_log.datatime)
            ...
            results.append({
                ...
                "created_at": list_date_format(doc.created_at),
                "updated_at": list_date_format(doc.updated_at),
                "processed_date": processed_date,
...
def add_document():
    try:
        ...
        if upload_type =="folder":
            upload_status=3
            file_name_for_db=f"fold_{datetime.now().strftime('%y%m%d%H%M%S%f')[:-3]}.pdf"
        else:
            upload_status=1
            file_name_for_db=f"maup_{datetime.now().strftime('%y%m%d%H%M%S%f')[:-3]}.pdf"
        ...
        # If file is an image, convert to PDF
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".rtf") as temp_rtf:
                temp_rtf.write(file.read())
                temp_rtf_path = temp_rtf.name
 
            temp_dir = tempfile.gettempdir()
 
            base_name = os.path.splitext(os.path.basename(temp_rtf_path))[0]
            temp_pdf_path = os.path.join(temp_dir, base_name + ".pdf")
            ...
            try:
                ...
                if not os.path.exists(temp_pdf_path):
                    raise Exception("RTF to PDF conversion failed. File not created.")
        ...
        # Upload to S3
        s3_path = upload_to_s3(io.BytesIO(pdf_file_bytes), filename,upload_status)
 
        # Save to DB
        doc = Document(
            doc_type_code="N/A",
            doc_filename=file_name_for_db,
            org_filename=filename,
            doc_file_path=s3_path,
            created_at=datetime.now(timezone.utc).astimezone(),
            updated_at=datetime.now(timezone.utc).astimezone(),
            doc_attach_status=0,
            upload_type=upload_status
        )
        db.session.add(doc)
        db.session.commit()
