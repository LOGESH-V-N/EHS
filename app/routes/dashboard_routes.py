...
def get_overview():
    try:
        ...
        processed_percentage = (processed_doc_count / overall_doc_count) * 100 if overall_doc_count > 0 else 0
        saved_percentage = (saved_doc_count / overall_doc_count) * 100 if overall_doc_count > 0 else 0
...
def get_document_type_count():
    try:
        ...
        for doc_type in doc_type_master:
            ...
            all_ehs_doc_qeued_count=all_ehs_doc.filter_by(doc_status=1,delete_status=0).count()
            all_ehs_doc_processed_count=all_ehs_doc.filter_by(doc_status=3,delete_status=0).count()
            all_ehs_doc_assigned_count=all_ehs_doc.filter_by(doc_status=4,delete_status=0).count()
            all_ehs_doc_saved_count=all_ehs_doc.filter_by(doc_status=5,delete_status=0).count()
            all_ehs_doc_archived_count=all_ehs_doc.filter_by(doc_status=6,delete_status=0).count()
            all_ehs_doc_query_count=all_ehs_doc.filter_by(doc_status=7,delete_status=0).count()
