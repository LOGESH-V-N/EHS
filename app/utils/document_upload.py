...
def upload_file_to_s3(file_obj, filename, content_type="application/octet-stream"):
    ...
    unique_name = f"{timestamp}_{uuid.uuid4()}_{new_filename}"
...
def mail_upload_to_s3(file_obj, filename,timestamp,system_generated):
    s3 = boto3.client(
        "s3",
        aws_access_key_id=current_app.config["AWS_ACCESS_KEY"],
        aws_secret_access_key=current_app.config["AWS_SECRET_KEY"],
    ...
    if str(system_generated).lower() == "true":
    ...
    while True:
        ...
        try:
            ...
            counter += 1
            unique_name = f"{file_date}--{name}-{counter}{ext}"
